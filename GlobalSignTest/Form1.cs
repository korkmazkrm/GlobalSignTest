...
namespace GlobalSignTest
{
    public partial class Form1 : Form
    {
        ...
        private static readonly string ApiSecret = Config.ApiSecret;
        private static readonly string MtlsPfxPath = Config.MtlsPfxPath;
        private static readonly string MtlsPfxPassword = Config.MtlsPfxPassword;
        ...
        private static readonly string LogFilePath = Path.Combine(AppContext.BaseDirectory, "signing.log");
        ...
        private async void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                WriteLog($"Signing started for file: {_selectedPdfPath}");
                var result = await RunDssFlowAsync();
                WriteLog($"Signing completed successfully for file: {_selectedPdfPath}");
                MessageBox.Show(result);
            }
            catch (TaskCanceledException)
            {
                WriteLog("Request timeout during signing flow.");
                MessageBox.Show("İstek timeout oldu (30sn).");
            }
            catch (Exception ex)
            {
                WriteLog($"Signing failed: {ex}");
        ...
        private async Task<string> RunDssFlowAsync()
        {
            ...
            var signResult = await SignPdfAsync(
                client,
                ...
                _selectedPdfPath!,
                outputSignedPdfPath);
            var verificationSummary = VerifySignedPdf(signResult.OutputPath);
            resultBuilder.AppendLine();
            resultBuilder.AppendLine("İmzalama sonucu:");
            resultBuilder.AppendLine(signResult.Summary);
            resultBuilder.AppendLine();
            resultBuilder.AppendLine("Doğrulama sonucu:");
            resultBuilder.AppendLine(verificationSummary);
        ...
        private static async Task<SignResult> SignPdfAsync(
            HttpClient client,
            string accessToken,
        ...
        private static string VerifySignedPdf(string signedPdfPath)
        {
            ...
            if (signatureNames.Count == 0)
                return "PDF içinde imza bulunamadı.";
            ...
            foreach (var name in signatureNames)
            {
                ...
                resultBuilder.AppendLine($"{name}: {(isValid ? "VALID" : "INVALID")}");
            }
        ...
        private static AppConfig LoadConfig()
        {
            ...
            using var document = JsonDocument.Parse(json);
            if (!document.RootElement.TryGetProperty("GlobalSign", out var section))
                throw new InvalidOperationException("Config içinde `GlobalSign` bölümü bulunamadı.");

            return new AppConfig(
                ReadRequiredConfigValue(section, "ApiKey"),
                ReadRequiredConfigValue(section, "ApiSecret"),
                ReadRequiredConfigValue(section, "MtlsPfxPath"),
                ReadRequiredConfigValue(section, "MtlsPfxPassword"));
        }

        private static string ReadRequiredConfigValue(JsonElement section, string propertyName)
        {
            if (!section.TryGetProperty(propertyName, out var property) ||
                property.ValueKind != JsonValueKind.String ||
                string.IsNullOrWhiteSpace(property.GetString()))
            {
                throw new InvalidOperationException($"Config değeri eksik: GlobalSign:{propertyName}");
            }
        ...
        private sealed record SignResult(string OutputPath, string Summary);
        private sealed record AppConfig(string ApiKey, string ApiSecret, string MtlsPfxPath, string MtlsPfxPassword);
